...
/// Build v4.19+ FileAuthenticationStorage .info file content.
/// This is plain JSON stored at {LOCALAPPDATA}/CodeBuddyExtension/Data/Public/auth/workbuddy-desktop.info
fn build_info_file_json(account: &WorkbuddyAccount) -> String {
    let uid = account.uid.as_deref().unwrap_or("");
    let nickname = account.nickname.as_deref().unwrap_or("");
    let domain = account.domain.as_deref().unwrap_or("");
    let refresh_token = account.refresh_token.as_deref().unwrap_or("");
    ...
    let info = serde_json::json!({
        ...
        "auth": {
            "accessToken": account.access_token,
            "expiresIn": expires_at - now_ms,
            "refreshExpiresIn": expires_at - now_ms,
            "refreshToken": refresh_token,
            "tokenType": account.token_type.as_deref().unwrap_or("Bearer"),
            "notBeforePolicy": 0,
            "sessionState": "",
...
/// Inject v4.19+ FileAuthenticationStorage .info file for WorkBuddy.
fn inject_workbuddy_auth_info_file(account: &WorkbuddyAccount) -> Result<(), String> {
    let info_path = crate::modules::workbuddy_account::get_workbuddy_auth_info_path()
        .ok_or_else(|| "无法获取 WorkBuddy auth info 文件路径".to_string())?;

    if let Some(parent) = info_path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|e| format!("创建 auth info 目录失败：{}", e))?;
    }

    // Remove logout marker if present
    let logout_marker = format!("{}.logged-out", info_path.to_string_lossy());
    let logout_path = std::path::PathBuf::from(&logout_marker);
    if logout_path.exists() {
        std::fs::remove_file(&logout_path).ok();
    }

    let json = build_info_file_json(account);

    // Atomic write via temp file + rename
    let tmp_path = format!("{}.tmp", info_path.to_string_lossy());
    std::fs::write(&tmp_path, &json)
        .map_err(|e| format!("写入 auth info 临时文件失败：{}", e))?;
    std::fs::rename(&tmp_path, &info_path)
        .map_err(|e| format!("重命名 auth info 文件失败：{}", e))?;

    logger::log_info(&format!(
        "[WorkBuddy Inject] 已写入 auth info 文件：{}",
        info_path.display()
    ));
...
pub async fn inject_workbuddy_to_vscode(
    ...
    // v4.20.4+ 主存储：写入 .info 文件（明文 JSON）
    // 必须先写 .info，因为 v4.21.0 可能没有 state.vscdb
    if let Err(err) = inject_workbuddy_auth_info_file(&account) {
        return Err(format!("写入 auth info 文件失败：{}", err));
    }

    // v4.10.4 兼容：写入 state.vscdb（加密）
    // 如果 state.vscdb 不存在（纯 v4.21.0 用户），跳过
    ...
    match state_db_path {
        Some(ref db_path) if db_path.exists() => {
            if let Err(err) = crate::modules::vscode_inject::inject_secret_to_state_db_for_workbuddy(
                db_path,
                &db_key,
                &session_json,
            ) {
                // state.vscdb 注入失败不阻断（v4.21.0 不需要它）
                logger::log_warn(&format!(
                    "[WorkBuddy Switch] state.vscdb 注入失败（非致命）：{}",
                    err
                ));
            }
        }
        _ => {
            logger::log_info("[WorkBuddy Switch] state.vscdb 不存在，跳过旧版注入（仅使用 .info 文件）");
        }
    }
