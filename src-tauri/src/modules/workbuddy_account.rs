...
/// v4.19+ FileAuthenticationStorage path: {LOCALAPPDATA}/CodeBuddyExtension/Data/Public/auth/workbuddy-desktop.info
/// v4.21.0 的 authenticationId 由 product.json 中的 authentication.id 决定，
/// 不再硬编码 "workbuddy-desktop"。
/// 本函数优先扫描 auth 目录下已有的 .info 文件（WorkBuddy 首次运行后会创建），
/// 找不到时 fallback 到 "workbuddy-desktop.info"。
fn get_auth_dir() -> Option<PathBuf> {
    #[cfg(target_os = "windows")]
    {
...
/// 扫描 auth 目录，找到第一个有效的 .info 文件（排除 .logged-out、.tmp、时间戳备份）。
fn scan_existing_info_file() -> Option<PathBuf> {
    let auth_dir = get_auth_dir()?;
    if !auth_dir.exists() {
        return None;
    }
    let entries = fs::read_dir(&auth_dir).ok()?;
    for entry in entries.flatten() {
        let name = entry.file_name();
        let name_str = name.to_string_lossy();
        // 只匹配 *.info，排除 *.info.logged-out, *.info.tmp, *.info.2026-04-29T*.info 等
        if !name_str.ends_with(".info") {
            continue;
        }
        if name_str.contains(".logged-out") || name_str.contains(".tmp") {
            continue;
        }
        // 排除时间戳备份: pattern like name.2026-04-29T08-05-43.info
        // 这些通常是 clean() 产生的备份，文件名中包含 ISO 时间戳
        let core = &name_str[..name_str.len() - 5]; // remove ".info"
        if core.contains(".20") && core.len() > 30 {
            // 大概率是时间戳备份，跳过
            continue;
        }
        let path = entry.path();
...
pub fn get_workbuddy_auth_info_path() -> Option<PathBuf> {
    // 优先复用已有的 .info 文件（v4.21.0 可能使用不同的 authenticationId）
    if let Some(existing) = scan_existing_info_file() {
        return Some(existing);
    }
    // fallback: v4.20.4 默认文件名
    let auth_dir = get_auth_dir()?;
    Some(auth_dir.join("workbuddy-desktop.info"))
}
